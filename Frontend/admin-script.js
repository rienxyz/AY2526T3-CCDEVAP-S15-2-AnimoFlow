...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function checkAdminAccess() {
        ...
        // If admin token exists, verify it's valid
        ...
        if (userData) {
            try {
                ...
                // ONLY allow if role is explicitly 'admin'
                if (user.role === 'admin') {
    ...
    // Run strict admin check - if fails, code stops here
    if (!checkAdminAccess()) {
        return;
    }
    ...
    async function deleteReport(id) {
        ...
        try {
            const response = await fetch(`http://localhost:3999/api/report/id/${id}`, { method: 'DELETE' });
